👋 Sobre mí:
Sebastián Lombardi

Soy un profesional analítico y estratégico, actualmente desempeñándome como Data Analyst, con una trayectoria previa de cuatro años en el área de compras dentro de la industria automotriz. Mi perfil combina visión de negocio, pensamiento crítico y una fuerte orientación a la optimización de procesos y toma de decisiones basada en datos.

Este README tiene como objetivo presentar mi recorrido profesional, mi enfoque de trabajo y las motivaciones que impulsan mi desarrollo continuo en el mundo del análisis de datos.

🚀 Mi Trayectoria Profesional
Experiencia en Compras – Industria Automotriz

Durante cuatro años trabajé en el área de compras (procurement) en una compañía de gran escala dentro de la industria automotriz. En este rol desarrollé una comprensión profunda de:

Gestión de proveedores
Negociación estratégica
Análisis de costos
Evaluación de riesgos
Planificación y abastecimiento
Optimización de procesos

Trabajar en compras me permitió entender cómo las decisiones impactan directamente en la rentabilidad y competitividad del negocio. No se trataba solo de negociar precios, sino de analizar datos, proyectar escenarios, evaluar indicadores y tomar decisiones estratégicas con información concreta.

Fue en ese contexto donde descubrí que mi mayor fortaleza y motivación no estaba únicamente en la gestión operativa, sino en el análisis profundo de información para mejorar sistemas, procesos y resultados.

Transición hacia Data Analytics

Impulsado por mi interés genuino por los datos y la mejora continua, decidí orientar mi carrera hacia el análisis de datos. Actualmente trabajo como Data Analyst, rol que representa una evolución natural de mis habilidades analíticas y estratégicas.

En esta nueva etapa profesional:

Transformo datos en información accionable
Construyo dashboards e informes para la toma de decisiones
Analizo tendencias y patrones
Identifico oportunidades de mejora
Aporto claridad en entornos complejos

Mi experiencia previa en negocio me permite entender el contexto detrás de los números. No analizo datos de forma aislada: busco comprender el impacto real en la estrategia y los resultados de la organización.

🧠 Mi Perfil Profesional

Mi personalidad se caracteriza por un enfoque estratégico, racional y orientado a resultados. Algunos rasgos que definen mi forma de trabajar:

🔍 Pensamiento Analítico

Tengo facilidad para descomponer problemas complejos en partes manejables y estructuradas. Disfruto enfrentando desafíos intelectuales y encontrando soluciones eficientes.

🧩 Visión Sistémica

No me enfoco solo en la tarea puntual. Intento comprender cómo cada elemento impacta en el sistema completo. Esta perspectiva me permite proponer mejoras estructurales y sostenibles.

📈 Mejora Continua

Siempre estoy buscando optimizar procesos, automatizar tareas repetitivas y generar mayor eficiencia. La pregunta constante que me hago es:

¿Cómo puede hacerse esto mejor?

🎯 Orientación a Resultados

Me motiva trabajar con objetivos claros y medibles. Prefiero decisiones basadas en evidencia antes que intuiciones aisladas.

🛠 Autonomía y Responsabilidad

Trabajo de manera independiente, con disciplina y organización. Cuando se me asigna una tarea crítica, me aseguro de entregar resultados precisos y de calidad.

💼 Qué Aporto a un Equipo

Mi combinación de experiencia en negocio y análisis de datos me permite actuar como puente entre áreas técnicas y estratégicas. Puedo traducir necesidades de negocio en métricas claras y transformar datos en recomendaciones concretas.

Algunos de los valores que aporto:

Claridad en la toma de decisiones
Análisis estructurado y fundamentado
Capacidad para priorizar lo relevante
Comunicación directa y objetiva
Compromiso con la calidad

Además, tengo altos estándares personales, lo que me impulsa a buscar precisión y excelencia en cada proyecto.

📊 Mi Enfoque en Data

Para mí, el análisis de datos no es solo generar gráficos o reportes. Es:

Comprender el problema de negocio
Formular las preguntas correctas
Identificar las métricas clave
Analizar con criterio
Comunicar hallazgos de forma clara
Generar impacto real

Creo firmemente que los datos tienen valor cuando influyen en decisiones concretas.

📚 Aprendizaje Continuo

Uno de mis principales motores es el aprendizaje autodirigido. Disfruto profundizar en nuevas herramientas, metodologías y conceptos. Mantengo una mentalidad de crecimiento constante, buscando ampliar mis conocimientos técnicos y estratégicos.

Mi desarrollo profesional no se basa únicamente en adquirir habilidades técnicas, sino también en fortalecer:

Comunicación efectiva
Inteligencia emocional
Colaboración interdisciplinaria
Adaptabilidad

Entiendo que el verdadero impacto surge cuando el conocimiento técnico se combina con habilidades interpersonales sólidas.

🤝 Mi Forma de Trabajar

Valoro entornos donde:

Se fomenta el pensamiento crítico
Se cuestionan procesos para mejorarlos
Se prioriza la eficiencia
Se respeta la autonomía
Se promueve el aprendizaje

Prefiero conversaciones profundas y orientadas a soluciones antes que dinámicas superficiales. Mi estilo es directo y racional, pero siempre enfocado en generar valor.

🌱 Objetivos Profesionales

Mi objetivo es seguir consolidándome como profesional en el ámbito del análisis de datos, ampliando mi capacidad para:
Diseñar soluciones basadas en datos
Participar en decisiones estratégicas
Liderar iniciativas de optimización
Generar impacto medible en organizaciones
Busco proyectos desafiantes que me permitan pensar estratégicamente, analizar en profundidad y contribuir a mejoras significativas.


📬 Conectemos

Si te interesa intercambiar ideas sobre análisis de datos, estrategia, optimización de procesos o crecimiento profesional, estaré encantado de conectar.

Este repositorio refleja no solo proyectos técnicos, sino también una forma de pensar: estructurada, estratégica y orientada al impacto.
